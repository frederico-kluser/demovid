...
- **`templates/` is in neither `tsconfig`, so `npm run test:remotion` is its only signal.** It is React that demovid never compiles. Three real type errors shipped in the first version of that template with `npm run verify` green; the e2e installs the generated project and runs `tsc` inside it. Scope: `templates/remotion/**`.
