...
- **Assert the overlay against `clientWidth`, not `innerWidth`.** `innerWidth` includes the scrollbar gutter; comparing against it yields a false negative on every recording. Scope: the post-camera assertion.
