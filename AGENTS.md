...
- **Configuration is deterministic first, agent second, and cached in the project.** `scan.ts` answers most projects for free; `pi` (deepseek-v4-pro, thinking `xhigh` — there is no `max`) is asked only when `confident` is false or nothing is cached, and its answer lands in the target project's `.demovid.json`, fingerprinted by manifest content so `npm install` alone does not re-bill it. When the scan *was* confident its measured infra overrides the agent's; the agent is authoritative only for what nothing measured (auth, what is worth demonstrating). pi speaks NDJSON: the answer is the last `message_end`'s `content[]` entry of type `text`, never the last line. It runs with **write access**, so the git working tree is diffed before/after and anything it touched is named — demovid cannot undo those. Scope: `src/project/discover.ts`, `src/project/config.ts`.
